...
namespace App1
{
    ...
    public sealed partial class FreeMode : Page
    {
        ...
        public FreeMode()
        {
            ...
            localsettings.Values["oldScore"] = highestScore;
        ...
        public void shulff()
        {
            int correctPos = new Random().Next(0, 9);  // 随机生成匹配的颜色块的位置
            ...
            colorGrid.ItemsSource = myColors;  // 设置GridView的内容
        }
        ...
        private List<MyColor> getErrorColors()
        {
            ...
            for (int i = 0; i < 8; i++) {
                errorColors.Add(new MyColor(tmp[i][0], tmp[(i + ranshif) % 7][1]));
            }
        ...
        private void exam(object sender, PointerRoutedEventArgs e)
        {
            ...
            if (!timer.IsEnabled) {
                ...
                Uri uri = new Uri("ms-appx:///Assets/Rabbits/Card" + id.ToString() + ".png");
                ((Image)((Grid)sender).Children[1]).Source =
                    new BitmapImage(uri);
                return;
            }
        ...
        public void hide()
        {
            ...
            for (int i = 0; i < 9; i++)
            {
                myColors.Add(new MyColor("#FFEEDD82", ""));
